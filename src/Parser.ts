...
export class Parser {
  ...
  private parseTasks () {
    this._tasks = []
    const tasksSheetValues = SpreadsheetApp.getActive()
      .getSheetByName('Tâches')
      ?.getDataRange()
      .getValues()
    if (tasksSheetValues) {
      ...
      tasksSheetValues.forEach(row => {
        ...
        if (assignee) assignee.addTask(task)
      })
    }
  }
  ...
  private parseTopics () {
    this._topics = []
    const topicsSheetValues = SpreadsheetApp.getActive()
      .getSheetByName('Sujets')
      ?.getDataRange()
      .getValues()
    if (topicsSheetValues) {
  ...
  private parseMeetings () {
    this._meetings = []
    const meetingsSheetValues = SpreadsheetApp.getActive()
      .getSheetByName('Réunions')
      ?.getDataRange()
      .getValues()
    if (meetingsSheetValues) {
      ...
      meetingsSheetValues.forEach(row => {
        ...
        attendingAcronyms.forEach((acronym: string) => {
  ...
  private parsePeople () {
    this._people = []
    const peopleSheetValues = SpreadsheetApp.getActive()
      .getSheetByName('Personnes')
      ?.getDataRange()
      .getValues()
    if (peopleSheetValues) {
      peopleSheetValues.shift() // shift removes first line that contains headings
      peopleSheetValues.forEach(row =>
        this.people.push(new Person(row[0], row[1], row[2], row[3]))
      )
    }
  }
